...
namespace Mono
{
	...
	public class CSharpShell
    {
		...
		String GetLine(Boolean primary)
		{

			//var prompt = primary ? InteractiveBase.Prompt : InteractiveBase.ContinuationPrompt;
            var prompt = primary ? "PipesShell> " : InteractiveBase.ContinuationPrompt;
		...
		delegate String ReadLiner(Boolean myPrimary);

		void InitializeUsing()
		{
            Evaluate("using System; using System.Linq; using System.Collections; using System.Collections.Generic;");
            Evaluate("using de.ahzf.Pipes; using de.ahzf.Pipes.ExtensionMethods; using de.ahzf.blueprints; using de.ahzf.blueprints.Datastructures; using de.ahzf.blueprints.InMemoryGraph; using de.ahzf.Pipes.UnitTests;");
		}

        void InitTerminal(Boolean myShowBanner)
		{
#if ON_DOTNET
			is_unix = false;
			isatty = true;
...
//			Report.Stderr = Console.Out;
			...
			if (isatty && myShowBanner)
				Console.WriteLine ("PipesShell, type \"help;\" for help\n\nEnter statements below.");
		...
		protected virtual void LoadStartupFiles()
		{
            ...
            var dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "csharp");
