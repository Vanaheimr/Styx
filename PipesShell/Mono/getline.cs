...
namespace Mono.Terminal
{
	...
	public class LineEditor
    {
		...
		public class Completion
        {
			...
			public Completion (String myPrefix, String[] myResult)
			{
				_Prefix = myPrefix;
				_Result = myResult;
			}
		...
		public delegate Completion AutoCompleteHandler(String myText, Int32 myPosition);
		...
		public LineEditor(String name, Int32 myHistsize)
		{
			...
			history       = new History(name, myHistsize);
        ...
        void UpdateCursor(Int32 newpos)
		{
			if (cursor == newpos)
				return;
		...
		void CmdTabOrComplete()
		{
			...
			if (AutoCompleteEvent != null)
            {
				...
				if (complete)
                {
					...
					string [] completions = completion._Result;
					...
					mismatch:
                        ...
                        {
							Console.Write(completion._Prefix);
							Console.Write(s);
							Console.Write(' ');
						}
		...
		public String Edit(String myPrompt, String myInitial)
		{
			...
			Prompt = myPrompt;
			shown_prompt = myPrompt;
			InitText(myInitial);
			history.Append(myInitial);
